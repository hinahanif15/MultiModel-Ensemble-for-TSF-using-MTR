...
from sklearn.ensemble import HistGradientBoostingRegressor  #addedd hisgrm regressor
import sys
...
class C45Regressor(DecisionTreeRegressor):

    def __init__(self, min_samples_split=2, min_samples_leaf=1, max_depth=None, max_features=None):
        super().__init__(criterion='friedman_mse', splitter='best',
    ...
    def _gain_ratio(self, X, y, feature_idx):
        ...
        _, threshold = np.unique(x_values, return_counts=True)
        gain = self._entropy(y)
        split_entropy = 0
        for t in threshold:
            _, counts = np.unique(y[x_values <= t], return_counts=True)
            freqs = counts / len(y[x_values <= t])
            split_entropy += len(y[x_values <= t]) / len(y) * (-np.sum(freqs * np.log2(freqs)))
            _, counts = np.unique(y[x_values > t], return_counts=True)
            freqs = counts / len(y[x_values > t])
            split_entropy += len(y[x_values > t]) / len(y) * (-np.sum(freqs * np.log2(freqs)))
    ...
    def _best_split(self, X, y):
        ...
        for feature_idx in range(X.shape[1]):
            gains = self._gain_ratio(X, y, feature_idx)
            if gains > best_gain:
                best_gain = gains
...
def model_xgb(x_train,y_train,x_test,y_test,booster,eta,max_depth,colsample_bytree,subsample,min_child_weight,reg_lambda,num_boost):
    ...
    params = {'booster': booster,
              'objective':'reg:squarederror', #previously it was reg:linear which has been deprecated now,
              'eta': eta,
...
#here Hist regressor added. 
def model_hst(x_train,y_train,x_test,y_test):
    bst = HistGradientBoostingRegressor(learning_rate = 0.03, loss='squared_error',random_state= 7,max_depth= 3,l2_regularization=0.7).fit(x_train,y_train)
    predict = bst.predict(x_test)
    return predict

#here RF regressor added. 

def model_rf(x_train,y_train,x_test,y_test):
    bst = RandomForestRegressor(criterion='squared_error',random_state= 7,max_depth= 3,n_estimators=30,n_jobs = -1).fit(x_train,y_train)
    predict = bst.predict(x_test)
    return predict
...
def get_linjie_matrix(x_train,y_train):
    ...
    for rr in range(y_train.shape[1]):
        ...
        for qq in range(num_epch):
            #previously it was DecisionTreeRegressor replaced with c45egressor
            x_train = x_train.astype(np.float32)  # Convert X to float32
            
            # Create an instance of C45Regressor with desired parameters
            estimator = C45Regressor(max_depth=config.all_config[text_file].get('adjacency_cart_depth'))
            # Fit the estimator with training data
            estimator.fit(x_train, delta_loss)
...
#    text_file = 'atp1d'
    ...
    kf = KFold(n_splits=config.all_config[text_file].get('paper_kFold'),shuffle=False)
        ...
        for i in range(label.shape[1]):
            ...
            for train_index_2, test_index_2 in kf_2.split(x_train):
                ...
                if text_file in ['sf1','sf2']:
                    ...
                    #for hist function settings
                    r1 = model_rf(x_train_2,y_train_2[:,i],x_test_2,y_test_2[:,i])
            ...
            if text_file in ['sf1','sf2']:
                ...
                predicted_test = model_rf(x_train,s_y_train[:,i],x_test,None)
